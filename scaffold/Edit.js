...
class [+up_singular+]Edit extends React.Component {
  ...
  onSuccess(response){
    const { id } = response
    this.props.history.push('/[+plural+]/' + id, this.state);
    this.props.notifySuccess("[+up_singular+] updated successfully")
  }
  ...
  onClick(){
    API.update('[+plural+]', this.state.[+singular+].id, this.state, this.onSuccess, this.onFailure)
  }
        ...
        <GridItem xs={12} sm={12} md={8}>
          <Card>
            <CardHeader color="primary">
              <h4 className={classes.cardTitleWhite}>Edit [+up_singular+] {[+singular+].id}</h4>
        ...
        <GridItem xs={12} sm={12} md={4}>
          <Card profile>
            ...
            <CardBody profile>
              ...
              <Button color="primary" onClick={this.show.bind(this, [+singular+])} >
                Show
              </Button>
            </CardBody>
          </Card>
